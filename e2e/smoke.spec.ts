...
test('catalog → estimate → quantity → discount → autosave → reload → clear', async ({ page }) => {
  ...
  const estimateQuantity = page.getByTestId('estimate-table').locator('input[title^="Количество"]:visible').first();
  ...
  await page.reload();
  const reloadedEstimateQuantity = page.getByTestId('estimate-table').locator('input[title^="Количество"]:visible').first();
  if (await mobileEstimateTab.isVisible().catch(() => false)) await mobileEstimateTab.click();
  await expect(page.getByRole('heading', { name: 'Позиции сметы' })).toBeVisible();
